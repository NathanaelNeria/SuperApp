...
class _LoginSignupPageState extends State<LoginSignupPage> {
  ...
  bool _isRegisterLoginOption;

  final connector = createPushConnector();

  // to save locally
  DatabaseHelper helper = DatabaseHelper();
  UserProfile userProfile= new UserProfile('', '', '', '', '', ''); // Step 1. Masukin userProfile
  TextEditingController emailController = TextEditingController(); // Step 2. assign controller ke email dan password
  ...
  StorageReference _reference = FirebaseStorage.instance.ref().child('myimage_login.jpg');
  ...
  List<UserProfile> listFirestoreUsers;
      ...
      if (validateAndSave()) {
        ...
        try {
          if (_isLoginForm) {
            ...
            userId = await widget.auth.signUp(_email, _password); // 1. illegible registration: auth ok, get userId
            ...
            var imageBytesFile = _imageFile.readAsBytesSync(); // 3. Convert registration image to base64 (to be saved locally later)
            ...
            createFirestoreData(userProfile.uid, userProfile.userDisplayName, userProfile.userEmail, userProfile.userPassword, userProfile.date);
//            // 4. b. sign out user from auth
//            await widget.auth.signOut();
            // 5. save locally
            Future<Database> dbFuture = helper.initializeDatabase();
            dbFuture.then((database) {
              Future<List<UserProfile>> userProfileListFuture = helper.getUserProfileList();
              userProfileListFuture.then((userProfileList){
                Future <int> resultDeleteAll = helper.deleteAllUserProfile();
                Future <int> resultInsert2 = helper.insertUserProfile(userProfile);
                if (resultInsert2 != 0) { // success
...
// 6. save the selfie picture to local
            ...
            Future<File> newImage = _imageFile.copy('${systemTempDir.path}/$userId.jpg');
          ...
          if (listFirestoreUsers.length>5) {
            String uidToDelete = listFirestoreUsers.elementAt(0).uid; //Directory systemTempDir = Directory.systemTemp;

            // 7.a. delete 1st (earliest) user auth
            await deleteFirstAuthUser(listFirestoreUsers.elementAt(0));

            // 7.b. delete from  firestore cloud
            QuerySnapshot querySnapshot = await Firestore.instance.collection("UserCRUD").getDocuments();
            await Firestore.instance.collection("UserCRUD").document(uidToDelete).delete();

            // 7.c. delete local image
            File photoToDelete = File('${systemTempDir.path}/$uidToDelete.jpg');
  ...
  Future<void> deleteFirstAuthUser(UserProfile userToDelete) async {
    await widget.auth.signOut();
    String uidToDelete = await widget.auth.signIn(userToDelete.userEmail, userToDelete.userPassword);
  ...
  Future getImage() async {
    if (_isRegisterLoginOption) { // kalau halaman depan
      //_getThingsOnStartup(); // TODO MAYA: Mestinya load semua foto
      faceRecognitionLogin();
    } else { // kalau registration mau upload image
      ...
      if(registerSelfieimage != null) {
        ...
        if (cropped!=null) {
          ...
          if (faceMatchFound1==true) {
            _showAlertDialog('Face Already Registered!', 'Same person cannot have two account! Please go to login page to login');
          ...
          {
            _showAlertDialog('Photo upload cancelled', 'Your photo has not been saved');
          }
        _errorMessage='';
//        var result = await FlutterImageCompress.compressAndGetFile(
//          image.path, image.+'/compressed.jpg',
//          quality: 80,
//        );
      }
...
//          StorageFileDownloadTask task = referenceTemp.writeToFile(currentDownloadedUserImage);
          ...
          Directory systemTempDir = Directory.systemTemp;
          File currentUserImage = File('${systemTempDir.path}/$currentUserUid.jpg');
  ...
  Future<bool> compareTwoImages(File image1, File image2) async {
    ...
    try {
      var streamIdCard = http.ByteStream(
          DelegatingStream.typed(image1.openRead()));
      var lengthIdCard = await image1.length();
      var streamProfile = http.ByteStream(
          DelegatingStream.typed(image2.openRead()));
...
//        return Future<bool>.value(false);
      ...
      debugPrint("Error $e");
      //return Future<bool>.value(false);
      return await false; // TODO MAYA: ini mestinya keluarin error message
    }
//    setState(() {
//      _isLoading = false;
//    });
  ...
  faceRecognitionLogin () async {
    ...
    for (int i=0; i<listFirestoreUsers.length; i++) {
      if (!faceMatchFound1) {
        ...
        // 2b. compare current selfie with the user[i]
        faceMatchFound1= await compareTwoImages(loginFaceFile, currentUserImage);
        firestoreUserProfile=(faceMatchFound1)? listFirestoreUsers.elementAt(i):null;
      }
    }
    ...
    switch (faceMatchFound1) {
      case true:
        String tempUserId=await widget.auth.signIn(firestoreUserProfile.userEmail, firestoreUserProfile.userPassword);
        print('Signed in: $tempUserId');
        widget.loginCallback();
        break;
      case false:
        (loginFaceFile==null)? print(''):_showAlertDialog("User not found!", "Please register first");
        break;
    }
...
//  }
  ...
  // Firestore
  // ###
  void createFirestoreData(String uid, String displayName, String userEmail, String userPassword, String registerDate) async {
...
//    }
  ...
  Future downloadAllFirestore() async {
    ...
    for (int i = 0; i < querySnapshot.documents.length; i++) {
      ...
      Map<String, dynamic> tempMap = a.data;
      String tempUid=a.documentID;
      ...
      //print(a.documentID);
      tempUserProfile=new UserProfile.withId(i,tempUid, tempDisplayName, tempUserEmail, tempUserPassword, tempRegisterDate);

      // download the user's photo
      // 2. Download the image of user[i]
      StorageReference referenceTemp = FirebaseStorage.instance.ref().child(tempUid+'.jpg');
      String downloadAddressTemp=await referenceTemp.getDownloadURL();
      Response downloadDataTemp = await get(downloadAddressTemp);
      Directory systemTempDir = Directory.systemTemp;
      File currentDownloadedUserImage = File('${systemTempDir.path}/$tempUid.jpg'); // image file dari user[i]
  ...
  Future<bool> _onWillPop() async {
    return (await showDialog(
      ...
      builder: (context) => new AlertDialog(
        ...
        content: new Text(
            (_isRegisterLoginOption)? 'Do you want to exit IST Demo?':'Do you want to leave this page?'),
        actions: <Widget>[
          new FlatButton(
